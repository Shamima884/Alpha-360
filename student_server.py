...
"""
═══════════════════════════════════════════════════════════════════════
 Meridian — STUDENT PANEL ONLY server (Render-ready)

 Serves *only* the student portal:
 ...
 Everything else — the admin control panel (admin/), the faculty
 portal (faculty/) and their scripts — returns 404, so this deployment
 exposes the student panel and nothing else.

 Pure Python standard library: no dependencies, no build step.

 RUN LOCALLY
     python student_server.py            →  http://localhost:8080

 DEPLOY (Render)
     startCommand: python student_server.py
     PORT is injected by Render automatically.

 OPTIONAL ACCESS PROTECTION (recommended for a public URL)
     STUDENT_USER=<user>  STUDENT_PASS=<pass>
...
NO_CACHE_PATHS = {"", "/", "/student", "/student/index.html"}

STUDENT_USER = os.environ.get("STUDENT_USER", "").strip()
STUDENT_PASS = os.environ.get("STUDENT_PASS", "")
AUTH_ON = bool(STUDENT_USER and STUDENT_PASS)

PORT = int(os.environ.get("PORT", "8080"))

SERVICE_NAME = "meridian-student"
...
<html lang="en"><head><meta charset="UTF-8"><title>Not found · Meridian Student</title>
<style>
 ...
 <div class="card">
  ...
  <p>This deployment serves the <strong>Meridian Student Portal</strong> only.<br>
     The page <code>__PATH__</code> is not part of it.</p>
  <a href="/">← Open the Student Portal</a>
 </div>
</body></html>"""


class StudentPanelHandler(BaseHTTPRequestHandler):
    server_version = "meridian-student/1.0"
    ...
    def _authorized(self):
        ...
        return (hmac.compare_digest(user, STUDENT_USER)
                and hmac.compare_digest(password, STUDENT_PASS))

    def _challenge(self):
        self._send(401, "Authentication required\n", "text/plain; charset=utf-8",
                   {"WWW-Authenticate": 'Basic realm="Meridian Student", charset="UTF-8"'},
                   no_store=True)
    ...
    def _handle(self):
        ...
        if path == "/healthz":
            self._send(200,
                       '{"ok":true,"service":"%s","scope":"student-panel-only","auth":%s}'
...
def main():
    httpd = ThreadingHTTPServer(("0.0.0.0", PORT), StudentPanelHandler)
    print("-" * 62, flush=True)
    print("  Meridian Student Portal  (student-only deployment)")
    print("  URL              http://0.0.0.0:%d" % PORT)
    print("  Access control   %s" % ("ON  (HTTP Basic Auth)" if AUTH_ON
                                     else "OFF - set STUDENT_USER + STUDENT_PASS"))
    print("  Serving          student/index.html - student/js/student.js")
    print("                   shared/css/{styles,admin}.css - shared/js/{api,data,charts,ui}.js")
    print("  Not served       admin/ - faculty/ - backend/ - student_server.py")
    print("-" * 62, flush=True)
    try:
        httpd.serve_forever()
